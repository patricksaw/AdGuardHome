...
interface FilteredAlertDomainsProps {
    topFilteredAlertDomains: unknown[];
...
const FilteredAlertDomains = ({
    t,
    refreshButton,
    topFilteredAlertDomains,
    subtitle,
    numFilteredAlert,
}: FilteredAlertDomainsProps) => (
    <Card title={t('top_filtered_alert_domains')} subtitle={subtitle} bodyType="card-table" refresh={refreshButton}>
        <ReactTable
            data={topFilteredAlertDomains.map(({ name: domain, count }: any) => ({
                domain,
                count,
            }))}
            columns={[
                ...
                },
                {
                    ...
                    accessor: 'count',
                    maxWidth: 190,
                    Cell: CountCell(numFilteredAlert),
                },
            ]}
...
export default withTranslation()(FilteredAlertDomains);
