...
interface ParentalAlertDomainsProps {
    topParentalAlertDomains: unknown[];
...
const ParentalAlertDomains = ({
    t,
    refreshButton,
    topParentalAlertDomains,
    subtitle,
    numParentalAlert,
}: ParentalAlertDomainsProps) => (
    <Card title={t('top_parental_alert_domains')} subtitle={subtitle} bodyType="card-table" refresh={refreshButton}>
        <ReactTable
            data={topParentalAlertDomains.map(({ name: domain, count }: any) => ({
                domain,
                count,
            }))}
            columns={[
                ...
                },
                {
                    ...
                    accessor: 'count',
                    maxWidth: 190,
                    Cell: CountCell(numParentalAlert),
                },
            ]}
...
export default withTranslation()(ParentalAlertDomains);
